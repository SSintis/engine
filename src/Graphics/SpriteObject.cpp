...
SpriteObject::SpriteObject(const std::string& title, float axisX, float axisY, float scale){
...
void SpriteObject::updateAnimation(Rotation rot){
    if(rot == Rotation::UP){
        ...
        sprite.setTextureRect(sf::IntRect(0, 2 * spriteHeight, spriteWidth, spriteHeight));
    }
    else if(rot == Rotation::LEFT){
        sprite.setTextureRect(sf::IntRect(0, spriteHeight, spriteWidth, spriteHeight));
    }
